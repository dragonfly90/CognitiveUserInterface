...
class Actions {
	...
	Action runTaskOnlyAction, stopAction, resumeAction;
	...
	Actions (final Core core, final Frame frame)
	{
		...
		runTaskOnlyAction = new AbstractAction ("Run SIMCog Task",
	...
	void update() {
		...
		runTaskOnlyAction.setEnabled(runEnabled);
		stopAction.setEnabled(core.hasLock(frame));
		//resumeAction.setEnabled(runEnabled && frame.getModel()!= null
		//		&& !frame.getModel().isDone());
		resumeAction.setEnabled(false); // TEH Temporarily disabled resume until it is updated for SIMCog.
