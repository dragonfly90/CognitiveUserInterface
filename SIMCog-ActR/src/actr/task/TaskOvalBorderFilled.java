...
/**
 * The class that defines an oval that is filled and has a boundary
 * that can be up to max(width, height) / 2 - 1.
 * 
 * @author Tim Halverson
 */
public class TaskOvalBorderFilled extends TaskOvalBorder {
	...
	public TaskOvalBorderFilled(int x, int y, int width, int height, int thickness) {
		this(x, y, width, height, thickness, NamedColor.WHITE, NamedColor.BLACK);
	}
	
	public TaskOvalBorderFilled(int x, int y, int width, int height, int thickness,
			boolean fillsIsPrimary) {
		this(x, y, width, height, thickness, NamedColor.WHITE, NamedColor.BLACK, fillsIsPrimary);
	}
	...
	{		
		...
		g.fillOval(this.thickness - 1, this.thickness - 1, getWidth() - 2*this.thickness + 1, getHeight() - 2*this.thickness + 1);
	}
