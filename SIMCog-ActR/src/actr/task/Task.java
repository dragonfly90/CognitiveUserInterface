...
public class Task extends JPanel {
	 ...
	 * In the case of running a SIMCog-based simulation, this method is called when the
	 * model is stopped.
	 */
	public void finish() {
	}
	...
	public void processDisplay() {
		...
		for (int i = 0; i < components.length; i++) {
			...
			if ((c instanceof TaskComponent) && c.isVisible()) {
				...
				Map<String,String> otherSlots = tc.getOtherSlots();
				model.getVision().addVisual (id, type, value, centerX(c),
						centerY(c), c.getWidth(), c.getHeight(), otherSlots);
			}
		}
	}
	...
	public void processDisplayNoClear() {
		...
		for (int i = 0; i < components.length; i++) {
			...
			if (c instanceof TaskComponent && c.isVisible()) {
				...
				model.getVision().updateValue(id, tc.getValue());
				model.getVision().updateOtherSlots(id, tc.getOtherSlots());
			}
		}
	}
	...
	/**
	 * Moves the task mouse to the given visual location.
	 * Note: This method is used by the move-cursor-anticipate
	 * motor action. See notes on this action in Motor.
	 * 
	 * @param locName the visual location to which the mouse is moved.
	 */
	public void moveMouseAticipate(int startX, int startY, Symbol locName) {
		Chunk loc = model.getVision().getVisualLocation(locName);
		int endX = loc.get(Symbol.get("screen-x")).toInt();
		int endY = loc.get(Symbol.get("screen-y")).toInt();
		
		double len = Math.sqrt(Math.pow(startX - endX, 2) + Math.pow(startY - endY, 2));
		double scale = (len + (len * 0.1)) / len;
		int newX = (int) (endX + (endX - startX) * scale);
		int newY = (int) (endY + (endY - startY) * scale);
	...
	/**
	 * Receives output by the model modules
	 * @param s string
	 */
	public void logModuleOutput(String s) {}
	
	/**
	 * Receives general output by the model
	 * @param s string
	 */
	public void logGeneralOutput(String s) {}
	
	/**
	 * Receives output from production execution
	 * (i.e., why-not type output)
	 * @param s string
	 */
	public void logWhyNotOutput(String s) {}
	
	/**
	 * Gets a string representation the task, defaulting to its name string.
	 * 
	 * @return a tab delimited string with the name of fields in the string returned by moduleUtilization()
	 */
	protected String moduleUtilizationHeader() {
		return "time\tgoalBusy\tretrievalBusy\tvislocBusy\tvisualBusy\taurlocBusy\tauralBusy\tmanualBusy\tvocalBusy\timaginalBusy\ttemporalBusy";
	}
	
	/**
	 * Outputs the busy state of all modules.
	 * 
	 * @return a tab delimited string with the fields specified in moduleUtilizationHeader()
	 */
	protected String moduleUtilization() {
		
		double time = this.getModel().getTime();
		
		boolean goalBusy = (this.getModel().getBuffers().getSlot(Symbol.goalState, Symbol.state) == Symbol.busy ? true : false);
		boolean retrievalBusy = (this.getModel().getBuffers().getSlot(Symbol.retrievalState, Symbol.state) == Symbol.busy ? true : false);
		boolean vislocBusy = (this.getModel().getBuffers().getSlot(Symbol.vislocState, Symbol.state) == Symbol.busy ? true : false);
		boolean visualBusy = (this.getModel().getBuffers().getSlot(Symbol.visualState, Symbol.state) == Symbol.busy ? true : false);
		boolean aurlocBusy = (this.getModel().getBuffers().getSlot(Symbol.aurlocState, Symbol.state) == Symbol.busy ? true : false);
		boolean auralBusy = (this.getModel().getBuffers().getSlot(Symbol.auralState, Symbol.state) == Symbol.busy ? true : false);
		boolean manualBusy = (this.getModel().getBuffers().getSlot(Symbol.manualState, Symbol.state) == Symbol.busy ? true : false);
		boolean vocalBusy = (this.getModel().getBuffers().getSlot(Symbol.vocalState, Symbol.state) == Symbol.busy ? true : false);
		boolean imaginalBusy = (this.getModel().getBuffers().getSlot(Symbol.imaginalState, Symbol.state) == Symbol.busy ? true : false);
		boolean temporalBusy = (this.getModel().getBuffers().getSlot(Symbol.temporalState, Symbol.state) == Symbol.busy ? true : false);
		
		return String.format("%f\t%b\t%b\t%b\t%b\t%b\t%b\t%b\t%b\t%b\t%b", time, goalBusy, retrievalBusy, vislocBusy, visualBusy, aurlocBusy, auralBusy, manualBusy, vocalBusy, imaginalBusy, temporalBusy);
	}
