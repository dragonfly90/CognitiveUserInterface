...
public class Model {
	...
	/**
	 * Gets the Production Rules file path.
	 *
	 * @return the file path for the production rules
	 */
	public String getProductionsFilePath() {
		return this.frame.getFilePath();
	}
	...
	public void run(boolean reset) {
		...
		while (!stop && (events.hasMoreEvents() || runUntilStop)) {
			...
			if (!event.getModule().equals("task")
					&& !event.getModule().equals("bold")
					&& !event.getModule().equals(""))
				this.logModuleOutput(event.getModule(), event.getDescription());
			event.action();
	...
	/**
	 * Runs a remote task for the simulation until <tt>stop()</tt> is explicitly called.
	 * This is step one of two for running a model in SIMCog.
	 * See startProcedural() below for the second step.
	 */
	public void runTaskOnly() {
		...
		while (!stop)
		{
			if(events.hasMoreEvents()) {
				...
				incrementalSleep ((long) Math.round(1));
			}
		}
		if (verboseTrace)
		{
	...
	/**
	 * Starts execution of the model for the simulation until <tt>stop()</tt> is explicitly called.
	 * This is step two of two for running a model in SIMCog.
	 * See runTaskOnly() above for the first step.
	 */
	public void startProcedural() {
		addEvent (new Event (this.getTime(), "procedural", "start") {
			public void action() {
				logWhyNot();
				procedural.findInstantiations (buffers);
			}});
	...
	/**
	 * Generates a "why not" log that is sent
	 * (line-by-line) to logWhyNotOutput().
	 */
	public void logWhyNot() {
		boolean old = procedural.whyNotTrace;
		procedural.whyNotTrace = false;
		procedural.whyNotLog = true;
		task.logWhyNotOutput("Begin whyNot\n");
		procedural.findInstantiations(buffers);
		task.logWhyNotOutput("End whyNot\n");
		procedural.whyNotLog = false;
		procedural.whyNotTrace = old;
	}
	...
	/**
	 * Logs a string to the Task for a particular module.
	 * 
	 * @param module
	 *            the module
	 * @param s
	 *            the message string
	 */
	public void logModuleOutput(String module, String s) {
		task.logModuleOutput(String.format("%9.3f   %-15s   %s", time, module, s));
	}

	/**
	 * Logs a string to the output panel.
	 * 
	 * @param s
	 *            the message string
	 */
	public void logGeneralOutput(String s) {
		task.logGeneralOutput(s);
	}

	/**
	 * Logs a string from "why not" trace to the output panel.
	 * 
	 * @param s
	 *            the message string
	 */
	public void logWhyNotOutput(String s) {
		task.logWhyNotOutput(s);
	}
