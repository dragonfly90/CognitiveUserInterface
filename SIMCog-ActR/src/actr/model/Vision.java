...
public class Vision extends Module {
	...
	private class VisualObject {
		...
		double encodingTime;
		Map<String, String> otherSlots;

		VisualObject(Symbol id, Symbol kind, Symbol value, int x, int y, int w,
				int h, double d, Map<String, String> otherSlots) {
			this.id = id;
			this.kind = kind;
			this.value = value;
			this.x = x;
		...
		public String toString() {
			String additional = "";
			for(Map.Entry<String, String> entry : this.otherSlots.entrySet())
			{
			    additional += " " + entry.getKey() + " " + entry.getValue();
			}
			
			return "(" + id + " kind " + kind + " screen-x " + x + " screen-y "
					+ y + " width " + w + " height " + h + " value " + value  + " :attended "
					+ (attended ? "t" : "nil") + additional + ")";
		}
	}
	 ...
	 *            value)
	 * @param otherSlots
	 *			  other slots of the visual object (e.g. the "color" slot value)
	 */
	public void addVisual(String id, String type, String value, int x, int y,
			int w, int h, double d, Map<String, String> otherSlots) {
		VisualObject vo = new VisualObject(Symbol.get(id), Symbol.get(type),
				Symbol.get(value), x, y, w, h, d, otherSlots);
		...
		if (model.bufferStuffing
			...
			if (newDist < curDist) {
				...
				if (model.getBuffers().get(Symbol.visloc) == null)
					model.logModuleOutput("vision", "unrequested ["
							+ visloc.getName().getString() + "]");
	 ...
	 *            value)
	 * @param otherSlots
	 *			  other slots and values of the visual object (e.g. the "color" slot value)
	 */
	public void addVisual(String id, String type, String value, int x, int y,
			int w, int h, Map<String, String> otherSlots) {
		addVisual(id, type, value, x, y, w, h, 0, otherSlots);
	}
	...
	public void addVisual(String id, String type, String value, int x, int y,
		...
		addVisual(id, type, value, x, y, w, h, d, Collections.<String,String>emptyMap());
	}
	...
	public void addVisual (String id, String type, String value, int x, int y,
			int w, int h)
	{
		addVisual(id, type, value, x, y, w, h, 0, Collections.<String,String>emptyMap());
	}
	...
	/**
	 * Updates the given visual object with new additional slots
	 *
	 * @param id
	 *			  the unique name of the visual object
	 * @param otherSlots
	 *			  additional information about the visual object (i.e., the "color" slot value)
	 */
	public void updateOtherSlots(String id, Map<String,String> otherSlots) {
		VisualObject vo = visicon.get(Symbol.get(id));
		if (vo != null)
		{
			vo.otherSlots = otherSlots;
		}
		model.noteTaskUpdated();
		// This call is probably not necessary, since the only time updateOtherSlots() is called is
		//	when moveVisual() is called.
	}
	...
	private boolean matches(Symbol slot, Symbol value, VisualObject vo) {
		...
		if(vo.otherSlots != null && slot.getString() != null) {
			// Check for "does not equal"
			if(slot.getString().charAt(0) == '-') {
				if(vo.otherSlots.containsKey(slot.getString().substring(1))) {
					if(vo.otherSlots.get(slot.getString().substring(1)).equals(value.getString()))
						return false;
					else
						return true;
				}
			} else {
				if(vo.otherSlots.containsKey(slot.getString())) {
					if(!vo.otherSlots.get(slot.getString()).equals(value.getString()))
						return false;
					else
						return true;
				}
	...
	void update() {
			...
			if (visloc != null) {
				if (model.verboseTrace)
					model.output("vision", "find-location ["
							+ visloc.getName().getString() + "]");
				model.logModuleOutput("vision", "find-location ["
						+ visloc.getName().getString() + "]");
				model.getBuffers().set(Symbol.visloc, visloc);
				model.getBuffers().setSlot(Symbol.vislocState, Symbol.state,
						Symbol.free);
				...
				model.logModuleOutput("vision", "error");
				model.getBuffers().setSlot(Symbol.vislocState, Symbol.state,
						Symbol.error);
				model.getBuffers().setSlot(Symbol.vislocState, Symbol.buffer,
						Symbol.empty);
		...
		if (request != null && request.isRequest()
			...
			for(Map.Entry<String, String> entry : vo.otherSlots.entrySet())
			{
			    visual.set(Symbol.get(entry.getKey()), Symbol.get(entry.getValue()));
			}
			if (model.verboseTrace)
				model.output("vision", "move-attention");
			model.logModuleOutput("vision", "move-attention");
