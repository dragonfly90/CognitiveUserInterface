...
class BufferCondition {
	...
	boolean testBufferChunk(Chunk bufferChunk, Instantiation inst) {
		for (int i = 0; i < slotConditions.size(); i++) {
			...
			if (model.getProcedural().whyNotTrace || model.getProcedural().whyNotLog) {
				...
				if (model.getProcedural().whyNotLog) {
					if (result && inst.size() > savedSize)
						model.logWhyNotOutput(slotCondition.toString(inst, null));
					else
						model.logWhyNotOutput(slotCondition.toString());
				}
				if (!result)
					return false;
			} else {
	...
	boolean test(Instantiation inst) {
		if (prefix == '!') {
			...
			if (model.getProcedural().whyNotLog)
				model.logWhyNotOutput(toStringFirstLine(null));
			...
			if (model.getProcedural().whyNotLog)
				model.logWhyNotOutput(toStringFirstLine(inst));
			if (bufferChunk == null)
				return false;
