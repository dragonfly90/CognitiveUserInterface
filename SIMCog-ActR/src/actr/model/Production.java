...
public class Production {
	...
	Instantiation instantiate(Buffers buffers) {
		...
		boolean savedWhyNotLog = model.getProcedural().whyNotLog;
		...
		if (prodGoalType != bufGoalType
			...
			model.getProcedural().whyNotLog = false;
		}

		if (model.getProcedural().whyNotTrace)
			model.output(name.getString());
		if (model.getProcedural().whyNotLog)
			model.logWhyNotOutput(name.getString());
		double instU = u
		...
		for (int i = 0; i < conditions.size(); i++) {
			...
			if (!bc.test(inst)) {
				...
				if (model.getProcedural().whyNotLog)
					model.logWhyNotOutput("   X instantiation failed\n");
				model.getProcedural().whyNotTrace = savedWhyNotTrace;
				model.getProcedural().whyNotLog = savedWhyNotLog;
				return null;
			}
		}
		...
		while (it.hasNext()) {
			...
			if (model.getProcedural().whyNotLog)
				model.logWhyNotOutput("   [delayed] " + dsc.buffer + ">");
		...
		if (model.getProcedural().whyNotLog)
			model.logWhyNotOutput("   * instantiation succeeded: " + inst + "\n");
		model.getProcedural().whyNotTrace = savedWhyNotTrace;
		model.getProcedural().whyNotLog = savedWhyNotLog;
		return inst;
	}
	...
	void fire(Instantiation inst) {
		...
		// TEH: I had a second condition (this.hasBufferCondition() in the next line.
		//		I don't recall why, so I removed it for testing.
		if (goal != null)
