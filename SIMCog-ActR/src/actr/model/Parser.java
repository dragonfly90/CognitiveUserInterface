	...
	void parse(Model model, String taskOverride) {
		try {
			...
			while (t.hasMoreTokens() || !t.getToken().equals("")) {
				...
				else if (t.getToken().equals("set-hand-location")) {
					t.advance();
					String hand = t.getToken();
					t.advance();
					int x = Integer.valueOf(t.getToken());
					t.advance();
					int y = Integer.valueOf(t.getToken());
					model.getMotor().moveHandTo(hand, x, y);
					t.advance();
