...
public class Declarative extends Module {
			...
			if (extendedMemory != null) {
				if (model.isVerbose())
					model.output("declarative", "extended memory: request "
							+ request);
				model.logModuleOutput("declarative", "extended memory: request "
						+ request);
				...
				if (chunk != null)
					model.logModuleOutput("declarative", "extended memory: found "
							+ chunk);
				else
					model.logModuleOutput("declarative", "extended memory: failure");
			}
	...
	void update() {
		...
		if (request != null && request.isRequest()) {
			...
			model.logModuleOutput("declarative", "start-retrieval");
